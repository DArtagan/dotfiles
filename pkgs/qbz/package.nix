...
rustPlatform.buildRustPackage (finalAttrs: {
  ...
  # Removed from nixpkgs 2026-09-18: upstream withdrew the repo, releases and
  # packages, so this URL 404s. The source and vendored crates are substituted
  # by hash from binary caches (cache.nixos.org, and the mini-nas `archive`
  # cache where they are pinned with GC disabled). Do NOT change the hashes.
  src = fetchFromGitHub {
    owner = "vicrodh";
    repo = "qbz";
