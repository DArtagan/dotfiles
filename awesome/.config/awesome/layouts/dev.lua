...
local function tile_group(cls, wa, fact, group)
    local height = "height"
    local width = "width"
    local x = "x"
    local y = "y"
...
function dev.arrange(param)
    ...
    for d = 1,2 do
        if place_master and nmaster > 0 then
            ...
            coord = coord + tile_group(cls, wa, data[0], {first=1, last=nmaster, coord = coord, size = size})
        end
        ...
        if not place_master and nother > 0 then
            ...
            for i = 1,ncol do
                ...
                coord = coord + tile_group(cls, wa, data[i], { first = first, last = last, coord = coord, size = size })
            end
        end
